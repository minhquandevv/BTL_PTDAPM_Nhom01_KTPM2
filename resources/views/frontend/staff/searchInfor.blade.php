@extends('frontend.component.layout-employee')
@section('content')
<form class="container mx-auto bg-white h-100 p-0" style="height:100vh">
    <div class="d-flex gap-5 align-items-center my-4 justify-content-between">
        <div>
            <a href="/bookinghome" class="position-absolute text-dark z-0" style="top:118px;left:45px;">
                <i class="fa-solid fa-chevron-left"></i>
            </a>
        </div>
        <div>
            <a href="#" class="text-decoration-none text-dark ">
                <p class="fs-4 fw-medium text-start ">Xe cắm trại/RV tại Half Moon Bay, California, Hoa Kỳ</p>
            </a>
        </div>
        <div>
           <a href="/edithomestay" class="text-decoration-none">
                <button type="button" class="btn border-0 ">
                    <i class="fa-regular fa-pen-to-square fa-2xl"></i>
                </button>
           </a>
            <a href="/deletehomestay" class="text-decoration-none">
                <button type="button" class="btn border-0 ">
                    <i class="fa-solid fa-trash-can fa-2xl" style="color: #ff0000;"></i>
                </button>
            </a>
        </div>
    </div>
    <div class="d-flex w-100 mb-5 mt-3 ">
        <div class="pe-3">
            <img class="card-img-top rounded-start" src="https://img.upanh.tv/2024/02/27/image-9.png" alt="" style="width:650px; height : 460px;" />
        </div>
        <div>
            <div class="d-flex">
                <img class="card-img-top me-2 mb-2" src="https://img.upanh.tv/2024/02/27/image-9.png" alt="" style="width: 320px; height:225px" />
                <img class="card-img-top rounded-end mb-2" src="https://img.upanh.tv/2024/02/27/image-9.png" alt="" style="width: 320px; height:225px " />
            </div>
            <div class="d-flex">
                <img class="card-img-top me-2" src="https://img.upanh.tv/2024/02/27/image-9.png" alt="" style="width: 320px; height:225px" />
                <img class="card-img-top rounded-end " src="https://img.upanh.tv/2024/02/27/image-9.png" alt="" style="width: 320px; height:225px" />
            </div>
        </div>
    </div>
    <div class="py-5">
        <div>
            <h2 class="fw-bold">Thông tin phòng</h2>
        </div>
        <div class="container mt-4 p-0 ">
            <div class="d-flex text-start my-3">
                <h4 class="col-lg-3 fw-medium fs-3">Mã phòng:</h4>
                <h4 class="col-lg-9 fw-medium fs-3">Mh4001</h4>
            </div>
            <div class="d-flex text-start my-3">
                <h4 class="col-lg-3 fw-medium fs-3">Tên phòng:</h4>
                <h4 class="col-lg-9 fw-medium fs-3">Xe cắm trại/ RV</h4>
            </div>
            <div class=" d-flex text-start my-3">
                <h4 class="col-lg-3 fw-medium fs-3">Giới thiệu phòng:</h4>
                <h4 class="col-lg-9 fw-medium fs-3">Khung cảnh lãng mạn riêng tư với tầm nhìn chưa từng có ra bờ
                    biển
                    và bãi
                    biển
                    Thái Bình Dương.
                    Chỗ ở
                    sang trọng trong một chiếc RV hoàn toàn mới, gần vịnh Half Moon và Santa Cruz</h4>
            </div>
            <div class="d-flex text-start my-3">
                <h4 class="col-lg-3 fw-medium fs-3">Trạng thái phòng:</h4>
                <h4 class="col-lg-9 fw-medium fs-3">Trống</h4>
            </div>
            <div class="d-flex text-start my-3">
                <h4 class="col-lg-3 fw-medium fs-3">Đơn giá:</h4>
                <h4 class="col-lg-9 fw-medium fs-3">6.800.000VND</h4>
            </div>
        </div>
</form>
@endsection
